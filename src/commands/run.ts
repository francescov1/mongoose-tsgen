...
// TODO ( last things )
// TODO: subsub docs working, enum strings, function params and return, pendingCharge, dont clear custom interfaces unless --fresh
...
export default abstract class Run extends Command {
    ...
    async run() {
        ...
        try {
            fullTemplate = generateAllInterfaces({ exceptions: [], modelsPath: args.path })
        }
        catch (error) {
            this.error(error)
        }
