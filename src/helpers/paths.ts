...
export const getModelsPaths = (basePath: string, extension: "js" | "ts"): string[] => {
    ...
    const searchPath = basePathEnd === "models" ? `**/!(index).${extension}` : `**/models/!(index).${extension}`;
    const modelsFolderPath = path.join(basePath, searchPath)

    const modelsPaths = glob.sync(modelsFolderPath, { ignore: "**/node_modules/**" })
...
export const getFullModelsPaths = (basePath: string, extension: "js" | "ts" = "ts"): string | string[] => {
