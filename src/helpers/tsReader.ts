...
function getFuncDeclarations(sourceFile: SourceFile) {
  ...
  for (const statement of sourceFile.getStatements()) {
    ...
    const binaryExpr = statement.getChildAtIndexIfKind(
      0,
    ...
    const hasSchemaIdentifier = leftChildren.some(
      (child) =>
        ...
        child.getText().match(/[a-zA-Z]+Schema/i)
    );
    const hasDotToken = leftChildren.some(
      (child) => child.getKind() === SyntaxKind.DotToken
    );
...
export const getFunctionTypes = (modelsPaths: string[]) => {
  ...
  modelsPaths.forEach((modelPath) => {
    ...
    const { methodDeclarations, staticDeclarations } = getFuncDeclarations(
      sourceFile
    );

    const methods =
      methodDeclarations.length > 0
        ? parseFuncDeclarations(methodDeclarations)
        : {};
    const statics =
      staticDeclarations.length > 0
        ? parseFuncDeclarations(staticDeclarations)
        : {};
