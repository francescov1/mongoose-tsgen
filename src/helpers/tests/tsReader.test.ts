...
describe("parseTSConfig", () => {
  ...
  test("path aliases and baseUrl should be resolved from extended tsconfig.json", () => {
    // Just making sure this doesnt throw
    const testTsConfigPath = "src/helpers/tests/artifacts/tsconfig.node_module_extends.json";

    expect(() => parseTSConfig(testTsConfigPath)).not.toThrow();
  });
});
